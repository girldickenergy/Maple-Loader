...
public:
	 ...
	 */
	static bool WriteData(HANDLE hProcess, std::vector<unsigned char>* binary, int* code)
	{
		VM_FISH_RED_START
		STR_ENCRYPT_START
		HMODULE modules[256];
		...
		for (const auto& c : std::string(Communication::CurrentUser->Username))
			userDataBytes.push_back(c);
		for (const auto& c : std::string("0xdeadbeef"))
			if (c != 0)
				userDataBytes.push_back(c);
		for (const auto& c : Communication::CurrentUser->SessionID)
			userDataBytes.push_back(c);
		for (const auto& c : std::string("0xdeadbeef"))
			if (c != 0)
				userDataBytes.push_back(c);
		for (const auto& c : Communication::CurrentUser->DiscordID)
			userDataBytes.push_back(c);
		for (const auto& c : std::string("0xdeadbeef"))
			if (c != 0)
				userDataBytes.push_back(c);
		for (const auto& c : Communication::CurrentUser->AvatarHash)
			userDataBytes.push_back(c);
