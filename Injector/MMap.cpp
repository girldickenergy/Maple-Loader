...
MMap::MMap(Process _process, std::vector<unsigned char> _file) :
    process{ OpenProcess(PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_VM_OPERATION | PROCESS_QUERY_INFORMATION, FALSE, _process.PID) },
    thread{ OpenThread(THREAD_SET_CONTEXT | THREAD_GET_CONTEXT | THREAD_SUSPEND_RESUME, FALSE, _process.TID) },
    file{ _file }, srcData{ file.data() }
{
    ...
    targetBase = reinterpret_cast<unsigned char*>(VirtualAllocEx(process, reinterpret_cast<void*>(oldOptHeader->ImageBase), oldOptHeader->SizeOfImage, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE));
    if (!targetBase)
    {
        targetBase = reinterpret_cast<unsigned char*>(VirtualAllocEx(process, nullptr, oldOptHeader->SizeOfImage, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE));
        if (!targetBase)
        {
...
constexpr std::array<unsigned char, sizeof...(Ts)> make_bytes(Ts&&... args) {
    return{ static_cast<unsigned char>(std::forward<Ts>(args))... };
}
...
static constexpr void __stdcall mapper(MANUAL_MAPPING_DATA* data)
{
    ...
    const auto base{ reinterpret_cast<unsigned char*>(data) };
    ...
    if (locationDelta)
    {
        ...
        while (relocData->VirtualAddress)
        {
            ...
            relocData = reinterpret_cast<IMAGE_BASE_RELOCATION*>(reinterpret_cast<unsigned char*>(relocData) + relocData->SizeOfBlock);
        }
    }
...
bool MMap::threadHijack()
{
    ...
    *reinterpret_cast<unsigned char**>(shellcode.data() + 0x26 + funcOffset) = reinterpret_cast<unsigned char*>(codeCave) + checkByteOffset;
    ...
    do
    {
        ReadProcessMemory(process, reinterpret_cast<unsigned char*>(codeCave) + checkByteOffset, &checkByte, 1, nullptr);
